...
def open_notepad_viewer_window():
    ...
    def display_mbr():
        """Display the MBR of a selected database file in a Text widget."""
        file_path = os.path.expanduser(r"~\AppData\Local\Microsoft\Edge\User Data")
        if not file_path:
            return
        ...
        try:
            with open(file_path, "rb") as file:
                mbr = file.read(512)
                mbr_hex = " ".join(f"{byte:02x}" for byte in mbr)
                text_area.delete(1.0, tk.END)
                text_area.insert(tk.END, f"MBR of {file_path}:\n\n{mbr_hex}")
        except Exception as e:
            messagebox.showerror("Error", f"Error reading MBR: {e}")

        mbr_button = ttk.Button(chrome_frame, text="Display MBR", command=display_mbr)
        mbr_button.pack(pady=5)
...
def history_parser():
        ...
        try:
            with open(file_path, "rb") as file:
                ...
                for i in range(0, len(data), 16):  # Display 16 bytes per line
                    ...
                    mbr+=(f"{i:04x}: {hex_values} {ascii_values}\n")
                return mbr
    ...
    def main():
        ...
        def display_edge_mbr():
            selected_profile = edge_profile_combo.get()
            if not selected_profile:
                messagebox.showwarning("Warning", "Please select an Edge profile.")
                return
            mbr_data=fetch_file_mbr(os.path.expanduser("~\\AppData\\Local\\Microsoft\\Edge\\User Data\\Default\\History"))
            display_data(mbr_data,text_area,mode="mbr")

        def display_chrome_mbr():
            selected_profile = chrome_profile_combo.get()
            if not selected_profile:
                messagebox.showwarning("Warning", "Please select an Chrome profile.")
                return
            mbr_data=fetch_file_mbr(os.path.expanduser("~\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\History"))
            display_data(mbr_data,text_area,mode="mbr")
        ...
        chrome_mbr_button = ttk.Button(chrome_frame, text="History MBR", command=display_chrome_mbr)
        chrome_mbr_button.pack(pady=10)
        ...
        edge_mbr_button = ttk.Button(edge_frame, text="History MBR", command=display_edge_mbr)
        edge_mbr_button.pack(pady=10)
